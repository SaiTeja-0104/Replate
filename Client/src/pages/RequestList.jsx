...
const DonationRequests = () => {
  const { getNgoDetails, getRequestsForDonation, getDonationDetails } =
  ...
  const refreshDonationData = async () => {
    setLoading(true); 
    try {
      const [updatedRequests, updatedDonation] = await Promise.all([
    ...
    const fetchAll = async () => {
      const enriched = await Promise.all(
        requests.map(async (req) => {
          if (!ngoCache[req._id]) {
            ngoCache[req._id] = await getNgoDetails(req._id);
          }
          return {
            ...req,
            ngo: ngoCache[req._id],
  ...
  if (loading) {
    return (
      <div className="flex flex-col sm:flex-row justify-center items-center min-h-screen">
        ...
        <p className="mt-10 sm:ml-10 sm:mt-0 text-orange-600 font-medium">Loading donation data...</p>
      </div>
    );
  }
    ...
    <div className="pt-6 font-pop px-4 sm:px-6 bg-[#fffdf7]">
      ...
      <div className="bg-[#fffdf7] border-[#FFD9B3] py-5 sm:px-6 flex items-center flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 space-y-2">
          ...
          <p className="flex items-center gap-2 text-sm mt-0.5 text-gray-700">
            ...
            {donation.status === "Expired"
              ...
              ? "Expires in " + getTimeDifference(donation.expiryDate)
              : "Donation Claimed"}
          </p>
        </div>
        ...
        <div className="flex items-center justify-center bg-[#FFF3E0] border border-[#FFCC80] rounded-xl py-5 px-6">
          <div className="text-center">
            ...
            <p className="text-2xl font-bold text-[#E65100]">{available}</p>
      ...
      <div className="bg-white py-5 px-6 rounded-t-4xl border-t border-r border-l w-full border-b border-orange-400">
        ...
        <p className="text-sm text-gray-600 pb-6">
          Total Requests:{" "}
          <span className="font-semibold text-[#E65100]">{requests.length}</span>
        </p>
            ...
            >
              <div className="flex flex-col md:flex-row justify-between gap-6">
                ...
                <div className="flex flex-col items-end justify-between">
                  ...
                  {req.status === "approved" && (
                    <button
                      onClick={() => startConversation(req.ngo?.data?._id)}
